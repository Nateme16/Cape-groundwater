function [npv damage IN net cost kgs kgi] = estuaryi4(control_s,c,taum,n,k,tau,s,r,cost_s,cost_in,R,IN0,B,Target,gamma)
...
for i=taum+1:taum+n; % this loop runs the control through time
    ...
    c2(ii,1)=(c(ii,i-tau(ii))-control_s1(ii,i-tau(ii)))*exp(-k*tau(ii));
    end
...
cost(i)=sum(cost_s.*(control_s1(1:s,i)).^2) + (cost_in.*control_s1(s+1,i).^2); %creates cost of year i based on choices in control matrix

kgs(i)=sum(control_s1(1:s,i));
